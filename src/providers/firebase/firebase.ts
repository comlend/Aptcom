		...
		return new Promise((resolve, reject) => {
			...
			var userData = {
				...
				profileurl: 'assets/imgs/imgPlaceholder.png'
							...
							firebase.auth().createUserWithEmailAndPassword(email, password).then((newUserCredential) => {
											console.log("new user",newUserCredential.user.uid);
											userData['uId'] = newUserCredential.user.uid;
											this.globals.userData = userData;
							firebase.database().ref('/users').child(newUserCredential.user.uid).set(userData).then(() => {
												var userRef = firebase.database().ref('/users').child(newUserCredential.user.uid);
							...
							this.uploadProfile(imageData, newUserCredential.user.uid).then((imageUrl) => {
